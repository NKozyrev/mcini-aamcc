void FragmentationInput(char* input_dirname, char* output_dirname, Int_t files, Int_t A, Int_t P_beam)
{
    ...
    fChain->Add(Form("%s/mcini_auau12AGeV_abla_branch1.root", input_dirname));

    TFile* ReadFile = new TFile("../input/auau_g4_option_12AGeV.root");
    ...
    TH1F* hEnergy =                             new TH1F("Energy",";E_{event}, GeV;counts",200,2357,2358);
    ...
    TH2F* hEnergyFrag_vs_EnergyEv =             new TH2F("E_frag_VS_E_event",";E_{event}, GeV;E_{frag}, GeV",300,0,3000,300,0,3000);
	...
	for (long i = 0; i < lNEvents; i++)
	{
        ...
        for (int j=0;j<fNpa;j++)
        {
            ...
            if (fParticle->GetPdg()>1e9) 
            {
                ...
                hP                                      ->Fill(fMomentum.Pz()/(fParticle->GetPdg()/10%1000)); //fMomentum.P()-P_beam*(fParticle->GetPdg()/10%1000-1)
                //if (fMomentum.Pz()/(fParticle->GetPdg()/10%1000) > 11.9 && fMomentum.Pz()/(fParticle->GetPdg()/10%1000) < 12.1) cout << "E_event: " <<  fEnergy << "; A: " << fParticle->GetPdg()/10%1000 << "; Z: " << fParticle->GetPdg()/10000%1000 << endl;
                ...
                hEnergyFrag_vs_EnergyEv                 ->Fill(fEnergy, fMomentum.E());
                //fEnergy=fEnergy+fMomentum.E();
            }       
            else if (fParticle->GetPdg()==2212) 
            {
                ...
                //if (fMomentum.Pz()/(fParticle->GetPdg()/10%1000) > 11.9 && fMomentum.Pz()/(fParticle->GetPdg()/10%1000) < 12.1) cout << "E_event: " <<  fEnergy << "; A: 1; Z: 0" << endl;
                hRapidity                               ->Fill(fMomentum.Rapidity());
                hPseudoRapidity                         ->Fill(fMomentum.PseudoRapidity());
            ...
            else if (fParticle->GetPdg()==2112) 
            {
                ...
                hRapidity_vs_Energy                     ->Fill(fMomentum.E(), fMomentum.Rapidity());
                hP                                      ->Fill(fMomentum.P());
                //if (fMomentum.Pz()/(fParticle->GetPdg()/10%1000) > 11.9 && fMomentum.Pz()/(fParticle->GetPdg()/10%1000) < 12.1) cout << "E_event: " <<  fEnergy << "; A: 1; Z: 1" << endl;
                hRapidity                               ->Fill(fMomentum.Rapidity());
                hPseudoRapidity                         ->Fill(fMomentum.PseudoRapidity());
                hPseudoRapidity_vs_A                    ->Fill(fMomentum.PseudoRapidity(),1);
                hEnergyFrag_vs_EnergyEv                 ->Fill(fEnergy, fMomentum.E());
    ...
    TH2F* hImpactParameter_vs_ExEn  =       new TH2F("B_VS_ExEn",";b, fm;E*/A, GeV",200,0,20,250,0,12);
    TH2F* hImpactParameter_vs_d  =       new TH2F("B_VS_ExEn",";b, fm; d, fm",200,0,20,200,1.,3.);
    TH2F* hImpactParameter_vs_A  =       new TH2F("A_VS_B",";b, fm; A_{pf}/A",200,0,20,198,0,1);
    TH2F* hRelA_vs_ExEn  =       new TH2F("ApfA_VS_ExEn",";E*/A, GeV; A_{pf}/A",360,0,12,198,0,1);
    TH2F* hNspect_vs_ExEn  =       new TH2F("Nspect_VS_ExEn",";Nspect;E*/A, GeV",100,0,100,250,0,12);
    ...
    TTree *tree1 = (TTree*) ReadFile->Get("MST: Clusters info");  
    ...
    for(int k = 0; k < tree->GetEntries(); k++){
        ...
        hImpactParameter_vs_A  -> Fill(b, double(sumA_proj)/197.);
    ...
    TCanvas* canvas = new TCanvas("canvas","NPP", 200, 200, 1200, 800);
    gPad->SetTickx();
    gPad->SetTicky();
    hImpactParameter           ->Draw("COLZ");
    canvas->Print("../output/histos/QA_test.pdf(");
    ...
    hEnergyFrag_vs_EnergyEv ->Draw("COLZ");
    canvas->Print("../output/histos/QA_test.pdf");
