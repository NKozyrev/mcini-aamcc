...
pad->cd(); pad->SetLogy(1);
  ...
  TH2D* np_histo = new TH2D("np_histo ","", 2000 ,-0.5 , 82.5, 36 ,2,1e3 );
...
nlines = ReadData("../gsi_c_mb.dat", xVal, 1., yVal, 1.);
TGraphErrors* PbPb_GSI_graph = new TGraphErrors(nlines-1, xVal, yVal, xErr, yErr);

  PbPb_GSI_graph->SetMarkerStyle(21);
  PbPb_GSI_graph->SetMarkerColor(kBlue-1);
...
norm = tree->GetEntries();
	for(int k = 0; k < tree->GetEntries(); k++){
		...
		if((Fheavy_z - Sheavy_z) < Fheavy_z*0.3333 && (mult  == 3)){
		for(int z = 0; z< Z->size(); z++ ){			
...
TLatex* sqrtS = new TLatex(4.5,1205, "PbC, 158A GeV");
...
leg2->AddEntry(PbPb_GSI_graph,"exp; Scheidenberger (2004)","lp");
leg2->Draw();

pad->Update();


gPad->Print("biasedPbC11MeV.png");
}
